...
developing_top_50_perc = top_frac(developing, 0.5, Life.expectancy)

library(MASS)

#Empty model with only beta0 (y-intercept)
model.empty = lm(Life.expectancy ~ 1, data = developing_top_50_perc)

#Model with all variables.
model.full = lm(Life.expectancy ~ ., data = developing_top_50_perc)
